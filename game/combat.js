...
export function fighter(x,kind){return {x,y:GROUND,vy:0,hp:100,kind,face:kind==='bolt'?1:-1,attack:null,block:false,stun:0,walking:false,anim:0,guard:0,think:.6,damageFlash:0};}
...
export function updateMatch(g,input,dt,aiEnabled=true){
 ...
 if(input.jump&&a.y===GROUND&&!a.attack&&!a.block&&a.stun===0)a.vy=-470;
 let cpuAxis=0;b.guard=Math.max(0,b.guard-dt);b.block=b.guard>0&&!b.attack&&b.stun===0;
 if(aiEnabled){let dist=Math.abs(a.x-b.x);cpuAxis=dist>168?b.face:dist<162?-b.face:0;b.think-=dt;if(b.think<=0){b.think=.35+g.rng()*.40;if(a.attack&&g.rng()<.30)b.guard=.4;else if(dist<185){b.block=false;startAttack(b,g.rng()<.40?'kick':'punch');}}}
 move(a,input.move||0,dt);move(b,cpuAxis*.76,dt);
